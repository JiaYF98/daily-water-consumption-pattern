...
def excel_split(file_name, cols_name):
    df = pd.read_excel('D:/日常/学习/大四/大四下/src/超声水表流量数据/原始数据/{}.xlsx'.format(file_name), index_col='时间')
    df.index = pd.to_datetime(df.index)  # 将索引转换为 datetime 类型
    new_list_date = sorted(list(set(df.index.date)))  # 只取日期部分
    for frame in new_list_date:
        excel_small = df[df.index.date == frame]
        ...
        for cols_len in range(len(cols_name)):
            excel_split_community = excel_small[cols_name[cols_len]]
